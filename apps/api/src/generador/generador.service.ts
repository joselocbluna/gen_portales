...
`;
    ...
    private parseSection(section: Section): string {
        ...
        const sectionStyles = section.styles || {} as any;
        const inlineStyle = [
    ...
    private buildStyleString(styles: any): string {
        if (!styles) return '';
        let str = '';
        if (styles.color) str += `color: ${styles.color}; `;
        if (styles.textAlign) str += `text-align: ${styles.textAlign}; `;
        if (styles.fontSize) str += `font-size: ${styles.fontSize}; `;
        return str.trim();
    }
}
